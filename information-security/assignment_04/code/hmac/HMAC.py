...
def read_stdin():
    """
    Read bytes from stdin and return them.
    """
...
def tiger_hash_inner_round(a, b, c, word, sboxes, mul):
    """
    Do one inner round of the tiger hash algorithm.

    `a`, `b` and `c` are the hash values.

    `word` is the word to be hashed.

    `sboxes` is the S boxes.
    ...
    s00 = sboxes[0][cBlocks[0]]
    s12 = sboxes[1][cBlocks[2]]
    s24 = sboxes[2][cBlocks[4]]
    s36 = sboxes[3][cBlocks[6]]
    s31 = sboxes[3][cBlocks[1]]
    s23 = sboxes[2][cBlocks[3]]
    s15 = sboxes[1][cBlocks[5]]
    s07 = sboxes[0][cBlocks[7]]
    # calculate the new hash values
...
def generate_key(block):
    """
    Generate the key block for the next round.
...
def tiger_hash_outer_round(a, b, c, block, mul, sBoxes, genNewKey=True):
    """
    Do one outer round of the tiger hash algorithm.
    ...
    if genNewKey:
        ...
        # return the new hash values and the keyBlock
        return a, b, c, newKeyBlock
    # if not, use the key block from the last round
    else:
        return a, b, c
...
def tiger_hash(data):
    """
    Hash the data using the tiger hash algorithm.

    `data` is the data to be hashed.
    """
    # divide the data into 64 byte blocks and add padding
    blocks = divide_blocks(data, 64, True)
...
# ------------------- END OF TIGER HASH ALGORITHM -------------------

def process_data(data):
    """
    Process the data and return the result.

    returns: tuple of (key, message)
    """
    # split the data at 0xFF
...
def apply_ipad_opad(ipad, opad, key):
    """
    Apply the ipad and opad to the key.
    """
    # repead ipad and opad to be 64 bytes long
    ipad = bytes([ipad] * 64)
    opad = bytes([opad] * 64)
    # pad the key with 0x00 if it is shorter than 64 bytes
    if len(key) < 64:
        key = key + bytes([0x00] * (64 - len(key)))
    # xor the key with ipad and opad
    key1 = bytes([a ^ b for a, b in zip(key, ipad)])
    key2 = bytes([a ^ b for a, b in zip(key, opad)])
    return key1, key2
...
def hmac(key, message):
    ...
    """
    key1 , key2 = apply_ipad_opad(IPAD, OPAD, key)
    # execute HMAC
    hash = tiger_hash(key2 + tiger_hash(key1 + message))
    return hash

def main():
    # this makes it possible to read from stdin when no filename is given on execution
    # execute with: "python HMAC.py [read_from]" to read from file
    if len(sys.argv) > 1:
        data = read_file(sys.argv[1])
    else:
        data = read_stdin()

    key, message = process_data(data)
    message = hmac(key, message)
    # or a one-liner if you want to be fancy but have horrible readability:
    # [message := tiger_hash(newKey + message) for newKey in [bytes([a ^ b for a, b in zip(key + bytes([0x00] * (64 - len(key))), bytes([pad] * 64))]) for pad in (IPAD, OPAD)]]


    # if desired output is also given in the run command, write to stdout
    # "python HMAC.py [read_in_from] [read_out_from]" to read input and desired output from file
    if len(sys.argv) > 2:
        # write correct hash to stdout if given using command
        print("generated hash: " + message.hex())
